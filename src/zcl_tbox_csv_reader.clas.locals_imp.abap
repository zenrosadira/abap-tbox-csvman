CLASS lcl_utils DEFINITION.
  PUBLIC SECTION.
    CLASS-METHODS is_elem
      IMPORTING i_data   TYPE data
      RETURNING VALUE(r) TYPE abap_bool.

    CLASS-METHODS is_table_of_elem
      IMPORTING i_data   TYPE data
      RETURNING VALUE(r) TYPE abap_bool.

    CLASS-METHODS is_table_of_one_component
      IMPORTING i_data   TYPE data
      RETURNING VALUE(r) TYPE abap_bool.

    CLASS-METHODS create_type_table_from_catalog
      IMPORTING i_catalog TYPE abap_component_view_tab
...
CLASS lcl_utils IMPLEMENTATION.
  ...
  METHOD is_table_of_elem.
    ...
    IF data_desc->kind <> cl_abap_typedescr=>kind_table.
      RETURN.
    ENDIF.

    DATA(tab_desc) = CAST cl_abap_tabledescr( data_desc ).
    IF tab_desc->get_table_line_type( )->kind <> cl_abap_typedescr=>kind_elem.
      RETURN.
    ENDIF.
  ...
  METHOD is_table_of_one_component.
    ...
    IF tab_line->kind <> cl_abap_typedescr=>kind_struct.
      RETURN.
    ENDIF.
