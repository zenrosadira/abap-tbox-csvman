...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD _to_external_format.
    ...
    IF dfies-convexit IS INITIAL.
      RETURN.
    ENDIF.

    cl_rsan_ut_conversion_exit=>convert_to_extern( EXPORTING i_fieldinfo      = CORRESPONDING #( dfies )
                                                             i_internal_value = i_sap_value
                                                   IMPORTING e_external_value = r_csv_value ).
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD _to_internal_format.
    ...
    cl_rsan_ut_conversion_exit=>convert_to_intern( EXPORTING  i_fieldinfo      = CORRESPONDING #( dfies )
                                                              i_external_value = i_csv_value
                                                   IMPORTING  e_internal_value = r_sap_value
                                                   EXCEPTIONS failed           = 1 ).
    IF sy-subrc = 0.
      m_conversion_exit = abap_true.
    ENDIF.
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  METHOD _write.
    ...
    IF m_conversion_exit = abap_true.
      RETURN.
    ENDIF.
